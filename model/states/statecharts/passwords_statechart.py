...
def init_input_state(root_state):
    ...
    passwords_statechart.add_state(state=input_state, parent=root_state.name)
    passwords_statechart.add_transition(Transition(source=input_state.name,
...
def create_correct_password_state(parent_state):
    ...
    passwords_statechart.add_state(state=correct_password_state, parent=parent_state.name)
    return correct_password_state


def create_blocked_pin_state(parent_state):
    blocked_pin_state = BasicState(name='blockedPin')
    passwords_statechart.add_state(state=blocked_pin_state, parent=parent_state.name)
...
def create_wrong_password_compound(parent_state, came_from_state):
    ...
    passwords_statechart.add_state(state=wrong_password_compound, parent=parent_state.name)
...
def create_wrong_password_default(name, parent_state, came_from_state):
    ...
    passwords_statechart.add_state(state=wrong_password_state, parent=parent_state.name)
    passwords_statechart.add_transition(Transition(source=wrong_password_state.name,
...
def create_can_input_state(parent_state, on_correct_input_state):
    ...
    passwords_statechart.add_state(state=can_input_state, parent=parent_state.name)
    passwords_statechart.add_transition(Transition(source=can_input_state.name, target=can_input_state.name, event='wrong_can'))
    passwords_statechart.add_transition(Transition(source=can_input_state.name,
...
def create_puk_input_state(parent_state, main_input_state):
    ...
    passwords_statechart.add_state(state=puk_input_state, parent=parent_state.name)
    passwords_statechart.add_transition(Transition(source=puk_input_state.name,
...
passwords_statechart = Statechart(name='BTOK passwords', preamble=INIT_STATE_PREAMBLE)
root_state = CompoundState(name='password', initial=INPUT_STATE_NAME)
passwords_statechart.add_state(state=root_state, parent=None)
